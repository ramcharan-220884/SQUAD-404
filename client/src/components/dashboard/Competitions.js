...
import { Plus, Trash2, Calendar, Trophy, AlertCircle, Loader2, Edit2, Eye, CheckCircle, Search, Users, Sun, Moon } from 'lucide-react';
  ...
  return (
    <div className={`p-4 md:p-8 space-y-6 ${isDark ? "feature-dark rounded-[2rem]" : ""}`}>
      ...
      {showDetailModal && selectedComp && (
        <div className="fixed inset-0 bg-black/60 z-[200] flex justify-center items-center p-2">
          <div className="bg-white dark:bg-slate-900 rounded-[1.5rem] w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 border dark:border-slate-800">
            <div className="bg-gradient-to-br from-orange-500 to-red-600 p-5 text-white relative">
              ...
              <h3 className="text-2xl font-black tracking-tight leading-tight uppercase mb-2">{selectedComp.title}</h3>
              <div className="flex flex-wrap gap-2">
                <span className="px-2 py-0.5 bg-white/20 rounded-md text-[9px] font-black uppercase tracking-wider backdrop-blur-md">Prize: {selectedComp.prize}</span>
                <span className="px-2 py-0.5 bg-white/20 rounded-md text-[9px] font-black uppercase tracking-wider backdrop-blur-md">Ends: {new Date(selectedComp.deadline).toLocaleDateString()}</span>
              </div>
            </div>
            <div className="p-5 max-h-[50vh] overflow-y-auto custom-scrollbar">
              <div className="prose dark:prose-invert max-w-none">
                <h4 className="text-[10px] font-black uppercase tracking-wider text-orange-500 mb-2">About Competition</h4>
                <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed font-medium whitespace-pre-wrap">{selectedComp.description || "No detailed description provided."}</p>
              </div>
            </div>
            <div className="p-5 bg-gray-50 dark:bg-slate-800/50 border-t dark:border-slate-800 flex justify-between items-center">
               <button onClick={() => setShowDetailModal(false)} className="px-4 py-2 border-2 border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-400 font-bold rounded-xl hover:bg-white dark:hover:bg-slate-800 transition-all text-xs">Close</button>
               {role === 'student' && !selectedComp.registered && (
                 <button onClick={() => { handleRegister(selectedComp.id); setShowDetailModal(false); }} className="px-6 py-2 bg-orange-600 text-white font-black rounded-xl hover:bg-orange-700 transition-all shadow-xl shadow-orange-500/20 active:scale-95 text-xs">Register Now</button>
               )}
            </div>
          </div>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 z-[200] flex justify-center items-center p-2">
          <div className="bg-white dark:bg-slate-900 p-5 rounded-[1.5rem] w-full max-w-lg shadow-2xl border border-gray-100 dark:border-slate-800">
            <h3 className="text-xl font-black text-gray-900 dark:text-white mb-4 uppercase tracking-tight">{isEditing ? 'Edit Competition' : 'Create Competition'}</h3>
            <form onSubmit={handleSave} className="space-y-3">
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1 block">Competition Title</label>
                <input required value={formData.title} onChange={e=>setFormData({...formData, title: e.target.value})} className="w-full bg-gray-50 dark:bg-slate-800 border-2 border-transparent focus:border-orange-500 p-2.5 rounded-xl outline-none font-bold text-gray-900 dark:text-gray-100 transition-all text-sm" />
              </div>
              <div className="flex gap-3">
                <div className="w-1/2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1 block">Prize Pool</label>
                    <input required value={formData.prize} onChange={e=>setFormData({...formData, prize: e.target.value})} className="w-full bg-gray-50 dark:bg-slate-800 border-2 border-transparent focus:border-orange-500 p-2.5 rounded-xl outline-none font-bold text-gray-900 dark:text-gray-100 transition-all text-sm" />
                </div>
                <div className="w-1/2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1 block">Deadline</label>
                    <input required type="date" value={formData.deadline} onChange={e=>setFormData({...formData, deadline: e.target.value})} className="w-full bg-gray-50 dark:bg-slate-800 border-2 border-transparent focus:border-orange-500 p-2.5 rounded-xl outline-none font-bold text-gray-900 dark:text-gray-100 transition-all text-sm" />
                </div>
              </div>
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1 block">Description</label>
                <textarea value={formData.description} onChange={e=>setFormData({...formData, description: e.target.value})} className="w-full bg-gray-50 dark:bg-slate-800 border-2 border-transparent focus:border-orange-500 p-2.5 rounded-xl outline-none font-bold text-gray-900 dark:text-gray-100 transition-all h-20 text-sm"></textarea>
              </div>
              <div className="flex justify-end gap-3 mt-4">
                <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 border rounded-xl font-bold text-xs">Cancel</button>
                <button disabled={actionLoading} type="submit" className="px-6 py-2 bg-orange-600 text-white rounded-xl font-bold hover:bg-orange-700 flex items-center gap-2 text-xs">
                  {actionLoading ? <Loader2 className="w-3 h-3 animate-spin"/> : null} {isEditing ? 'Save' : 'Publish'}
                </button>
              </div>
            </form>
          </div>
