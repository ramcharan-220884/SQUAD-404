...
const LS_PENDING  = "pendingCompetitions";
const LS_APPROVED = "approvedCompetitions";
...
export default function Competitions({ role = "student" }) {
  ...
  // Backend-fetched competitions
  const [backendComps, setBackendComps] = useState([]);
  // localStorage-approved competitions (student-submitted & admin-approved)
  const [lsApproved, setLsApproved] = useState(getLSApproved);
  // Pending student submissions (admin only)
  const [pendingComps, setPendingComps] = useState(getLSPending);
  ...
  const fetchComps = useCallback(async () => {
    try {
      ...
      setBackendComps(Array.isArray(data) ? data : (data.data || []));
    } catch (err) {
  ...
  // Sync pending from localStorage whenever it might change externally
  useEffect(() => {
    const sync = () => {
      setPendingComps(getLSPending());
  ...
  // ── Approved list (merges backend + localStorage-approved) ─────────────────
  const backendApproved = backendComps.filter(c => c.status === 'approved' || !c.status);
  const allApproved = [
    ...backendApproved,
    ...lsApproved.filter(lc => !backendApproved.some(bc => bc.id === lc.id))
  ];
  const filteredComps = allApproved.filter(c =>
    c.title.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // ── Student: register for backend competition ──────────────────────────────
  ...
  // ── Admin: delete backend competition ─────────────────────────────────────
  const handleDelete = async (id) => {
    if (role !== 'admin') return;
    if (!window.confirm("Delete this competition?")) return;
    try {
      await deleteCompetition(id);
      setBackendComps(prev => prev.filter(c => c.id !== id));
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (role === 'student') {
        ...
        const updated = [...getLSPending(), newComp];
        setLSPending(updated);
        setPendingComps(updated); // also update admin view if same session
  ...
  const handleEdit = (comp) => {
    ...
    setFormData({
      ...
      category: comp.category || comp.type || "Hackathon",
  ...
  const approveCompetition = (id) => {
    const pending = getLSPending();
    const compToApprove = pending.find(c => c.id === id);
    if (!compToApprove) return;

    const approved = { ...compToApprove, status: 'approved' };
    const newPending  = pending.filter(c => c.id !== id);
    const newApproved = [...getLSApproved(), approved];

    setLSPending(newPending);
    setLSApproved(newApproved);
    setPendingComps(newPending);
    setLsApproved(newApproved);

    showNotification("Competition approved — now visible to students!", "success", "admin");
  };

  // ── Admin: Reject student submission ───────────────────────────────────────
  const rejectCompetition = (id) => {
    const newPending = getLSPending().filter(c => c.id !== id);
    setLSPending(newPending);
    setPendingComps(newPending);
    showNotification("Competition rejected.", "error", "admin");
  };
            ...
            <div key={c.id} className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-slate-700 relative group transition-all hover:shadow-md hover:-translate-y-1">
              ...
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border ${getCategoryStyle(c.category || c.type)}`}>
                    {c.category || c.type}
                  </span>
                  {c.status === 'approved' && (
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-50 dark:border-slate-700">
                <div className="flex items-center gap-1.5 text-[10px] font-bold text-gray-400 uppercase tracking-widest flex-1">
                  ...
                  <span>{(c.date || c.deadline) ? new Date(c.date || c.deadline).toLocaleDateString() : 'TBD'}</span>
                </div>
              ...
              <div key={c.id} className="bg-orange-50/50 dark:bg-orange-900/10 p-6 rounded-[2rem] border border-orange-200/60 dark:border-orange-800/30 relative shadow-sm hover:shadow-md transition-all">
                ...
                {c.link && (
                  <a
                    ...
                    className="flex items-center gap-1.5 text-[10px] font-bold text-orange-600 mb-4 hover:underline"
                  >
                ...
                <div className="flex gap-3 mt-auto">
                  <button
                    onClick={() => approveCompetition(c.id)}
                  ...
                  <button
                    onClick={() => rejectCompetition(c.id)}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 border dark:border-slate-800">
            <div className="bg-gradient-to-br from-orange-500 to-red-600 p-8 text-white relative">
              ...
              <div className="flex flex-wrap gap-3">
                <span className="px-3 py-1 bg-white/20 rounded-lg text-[10px] font-black uppercase tracking-wider backdrop-blur-md">{selectedComp.category || selectedComp.type}</span>
                <span className="px-3 py-1 bg-white/20 rounded-lg text-[10px] font-black uppercase tracking-wider backdrop-blur-md">
                  {(selectedComp.date || selectedComp.deadline) ? new Date(selectedComp.date || selectedComp.deadline).toLocaleDateString() : 'TBD'}
                </span>
            ...
            <div className="p-8 max-h-[60vh] overflow-y-auto custom-scrollbar">
              <div className="prose dark:prose-invert max-w-none">
                ...
                {selectedComp.link && (
                  <div className="mt-6 p-4 bg-orange-50 dark:bg-orange-900/20 rounded-2xl border border-orange-100 dark:border-orange-800/50">
                    <a href={selectedComp.link.startsWith('http') ? selectedComp.link : `https://${selectedComp.link}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-orange-600 dark:text-orange-400 font-bold hover:underline">
            ...
            <div className="p-8 bg-gray-50 dark:bg-slate-800/50 border-t dark:border-slate-800 flex justify-between items-center">
              ...
              {role === 'student' && !selectedComp.registered && !selectedComp.status && (
