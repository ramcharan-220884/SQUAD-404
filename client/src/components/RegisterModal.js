...
const RegisterModal = ({ isOpen, onClose, onSwitchToLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        onClose();
        onSwitchToLogin(); // Send them to login page
      }, 3000);
    } catch (err) {
        ...
        <div className="p-6 lg:p-8 font-['Poppins']">
          ...
          {isSuccess ? (
            <div className="flex flex-col items-center justify-center py-10 text-center animate-in fade-in zoom-in duration-500">
              <div className="w-20 h-20 bg-green-50 rounded-full flex items-center justify-center text-green-500 mb-6 border-4 border-green-100/50">
              ...
              <p className="text-sm text-gray-500 font-medium max-w-[280px] leading-relaxed">
                Your account has been created and is now <span className="text-[#346b41] font-bold">pending admin approval.</span> You will be able to login once approved.
              </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={loading}
                className={`w-full rounded-2xl text-white text-sm font-extrabold uppercase tracking-[0.15em] shadow-xl shadow-[#346b41]/20 transition-all active:scale-[0.98] ${
                  loading
