...
export default function AdminDashboard() {
  ...
  const handleStudentAction = async (id, action) => {
    try {
      ...
      alert(`Error ${action}ing student: ${err.message}`);
    }
  };
  ...
  const handleRecruiterAction = async (id, action) => {
    try {
      ...
      alert(`Error ${action}ing recruiter: ${err.message}`);
    }
  };

  // Prepare UI stats from real data
  const UI_STATS = stats ? [
    {
      title: "Total Students",
    ...
    },
    {
      ...
      icon: <FileCheck className="w-6 h-6 text-green-600" />,
      change: stats.pendingApprovals > 0 ? "Action Required" : "Cleaned",
    },
    {
      title: "System Health",
      value: "99.9%",
    ...
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden">
      ...
      <aside className="w-64 bg-green-900 text-green-50 flex flex-col hidden md:flex shrink-0">
        ...
        <nav className="flex-1 mt-6 px-4 space-y-2 overflow-y-auto">
          <a
            href="#"
            className="flex items-center gap-3 px-4 py-3 bg-green-800 text-white rounded-xl shadow-inner font-medium transition-all"
          >
        ...
        <div className="p-4 mt-auto">
          <div className="bg-green-800 rounded-xl p-4 shadow-sm border border-green-700/50">
            ...
            <a
              href="/"
              className="block mt-4 text-center text-sm text-green-100 bg-green-900/50 hover:bg-green-700 py-2 rounded-lg font-medium transition-colors border border-green-700/50 hover:border-green-600"
            >
              Logout
            </a>
          </div>
      ...
      <main className="flex-1 overflow-y-auto w-full">
        ...
        <header className="bg-white shadow-sm px-6 py-4 flex items-center justify-between md:hidden sticky top-0 z-10">
          <h1 className="text-xl font-bold tracking-wider text-green-900 flex items-center gap-2">
            <span className="bg-green-900 text-white p-1 rounded text-sm">
              E
            </span>
            DUVATE
          </h1>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex gap-3">
              <button 
                ...
                disabled={refreshing}
                className="flex items-center gap-2 px-4 py-2.5 bg-white border-2 border-green-100 text-green-700 font-bold rounded-xl hover:bg-green-50 hover:border-green-200 transition-all shadow-sm disabled:opacity-50"
              >
              ...
              UI_STATS.map((stat, i) => (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-4 relative z-10">
                    ...
                    <span
                      className={`text-sm font-bold px-2.5 py-1 rounded-full ${stat.change === "Action Required" ? "bg-orange-100 text-orange-700" : "bg-green-100 text-green-700"}`}
                    >
                      {stat.change}
                    </span>
                  </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-white border-b border-gray-100 text-gray-500 text-xs uppercase tracking-widest">
                      ...
                      <th className="px-6 py-4 font-bold w-48 text-right">
                        Action
                      </th>
                    </tr>
                  </thead>
                      ...
                      pendingRecruiters.map((recruiter) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 flex gap-2 justify-end">
                            <button
                              onClick={() =>
                                handleRecruiterAction(recruiter.id, "Approve")
                              }
                              className="p-2 sm:px-3 sm:py-2 bg-green-50 text-green-600 hover:bg-green-600 hover:text-white rounded-lg flex items-center justify-center gap-1.5 font-bold transition-colors border border-green-200 hover:border-green-600"
                              title="Approve"
                            >
                            ...
                            <button
                              onClick={() =>
                                handleRecruiterAction(recruiter.id, "Reject")
                              }
                              className="p-2 sm:px-3 sm:py-2 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white rounded-lg flex items-center justify-center gap-1.5 font-bold transition-colors border border-red-200 hover:border-red-600"
                              title="Reject"
                            >
